...
public class Ntag_I2C_Commands extends I2C_Enabled_Commands {
   ...
   private static final int SRAM_SIZE = 64; //64bytes
   /*
   ...
   private boolean TimeOut = false; // timeout for GET_VERSION is 5ms
   ...
   public enum Register {
      Session((byte) 0xF8),         // read session register
      Session_PLUS((byte) 0xEC),   // read session register for plus product
      Configuration((byte) 0xE8),   // read configuration registers
      SRAM_Begin((byte) 0xF0),
      Capability_Container((byte) 0x03),
      User_memory_Begin((byte) 0x04),
      Custom_User_memory_Begin((byte) 0x0D), // 2(lang) + 64(text) + 1 = 67 => 9 bytes
      UID((byte) 0x00),
      AUTH0((byte) 0xE3),
      ACCESS((byte) 0xE4),
      PWD((byte) 0xE5),
...
/*		reader.SectorSelect((byte) 3);
   ...
   */
   ...
   public void writeEEPROM(byte[] data, WriteEEPROMListener listener) throws IOException, FormatException {
      ...
      byte[] temp;
      int index = 0;
      byte startAt = Register.User_memory_Begin.getValue();
      byte blockNr = startAt;
      SharedCodes.Companion.log("NI2C", "writeEEPROMCustom data:" + new String(data));
      SharedCodes.Companion.log("NI2C", "writeEEPROMCustom start at address:" + blockNr);
      ...
      for (index = 0; index < data.length && blockNr != 0; index += 4) {
         ...
         SharedCodes.Companion.log("write", temp + " " + blockNr);
   ...
   @Override
   public byte[] readEEPROMCustomBytes(int absStart, int absEnd) throws IOException,
           FormatException {
   ...
   @Override
   public void writeEEPROMCustomBytes(byte[] data, byte startAt) throws IOException, FormatException {
      reader.SectorSelect((byte) 0);
      byte[] temp;
      int index = 0;
      byte blockNr = startAt; // Register.Custom_User_memory_Begin.getValue();
      SharedCodes.Companion.log("NI2C", "writeEEPROMCustomBytes start at address:" + blockNr);
      for (index = 0; index < data.length && blockNr != 0; index += 4) {
         // NTAG I2C Plus sits the Config registers in Sector 0
         if (getProduct() == Prod.NTAG_I2C_2k_Plus && blockNr == (byte) 0xE2) {
            break;
         ...
         temp = Arrays.copyOfRange(data, index, index + 4);
         reader.write(temp, blockNr);
         SharedCodes.Companion.log("write", blockNr + ": " + SharedCodes.Companion.join(temp) );
         blockNr++;
      }
   }
   ...
   */
   @Override
   public void writeEEPROMCustom(byte[] data, WriteEEPROMListener listener) throws IOException, FormatException {
      if (data.length > getProduct().getMemsize()) {
         throw new IOException("Data is to long");
      }
      reader.SectorSelect((byte) 0);
      byte[] temp;
      int index = 0;
      byte startAt = NtagRawRecord.Companion.getSeekWrittenAt(); // Register.Custom_User_memory_Begin.getValue();
      byte blockNr = startAt;
      SharedCodes.Companion.log("NI2C", "writeEEPROMCustom start at address:" + blockNr);
      // write till all Data is written or the Block 0xFF was written(BlockNr
      // should be 0 then, because of the type byte)
    ...
    */
   @Override
   public byte[] readEEPROMCustom(int absStart, int absEnd) throws IOException,
           FormatException {
      int maxfetchsize = reader.getMaxTransceiveLength();
      int maxFastRead = (maxfetchsize - 2) / 4;
      int fetchStart = absStart;
   ...
   public void writeSRAM(byte[] data, R_W_Methods method, WriteSRAMListener listener) throws IOException,
      ...
      // note: gordianknot
      // does 64.0 indicates a SRAM_SIZE? if that is true then it must be replaced with:
      // float SRAM_FSIZE = SRAM_SIZE;
      float SRAM_FSIZE = SRAM_SIZE;
   ...
   public void writeNDEF(NdefMessage message, WriteEEPROMListener listener) throws IOException,
      ...
      SharedCodes.Companion.log("writeNDEF", "message:" + message);
      writeEEPROM(Ndef_message_byte, listener);
   }

   @Override
   public void writeNDEFCustom(NdefMessage message, WriteEEPROMListener listener) throws IOException,
           FormatException {
      byte[] Ndef_message_byte = createRawNdefTlv(message);
      SharedCodes.Companion.log("writeNDEFCustom", "message" + message);
      writeEEPROMCustom(Ndef_message_byte, listener);
   }
   ...
   public NdefMessage readNDEF() throws IOException, FormatException {
      ...
      SharedCodes.Companion.log("readNormal", "start at " + Register.User_memory_Begin.getValue());
    ...
    */
   @Override
   public NtagRawRecord readNDEFCustom() throws IOException, FormatException {
      int ndefsize;
      int tlvsize;
      int tlvPlusNdef;

      // get TLV  // note: [TLV] Tag-Length-Value
      byte startAt = NtagRawRecord.Companion.getSeekWrittenAt(); // Register.Custom_User_memory_Begin.getValue();
      byte[] tlv = readEEPROM(startAt, startAt + 3);
      SharedCodes.Companion.log("readCustom", "start at " + startAt);
      NdefMessage message = null;
      boolean isValidNdefMessage = tlv[0] == 0x03;
      ...
      // Read NDEF Message
      byte[] data = readEEPROM(startAt, startAt + (tlvPlusNdef / 4));

      SharedCodes.Companion.log("readNDEFCustom", "isValidNdefMessage:" + isValidNdefMessage);
      if (isValidNdefMessage){
         byte[] tempdata = Arrays.copyOf(data, data.length);
         // delete TLV
         tempdata = Arrays.copyOfRange(tempdata, tlvsize, tempdata.length);
         // delete end of String which is not part of the NDEF Message
         tempdata = Arrays.copyOf(tempdata, ndefsize);
         SharedCodes.Companion.log("readNDEFCustom", "data:" + bytesToHex(tempdata));
         message = new NdefMessage(tempdata);
      }

      // get the String out of the Message
      return new NtagRawRecord(tlvPlusNdef, tlv[0] == 0x03, tlvsize, ndefsize, data, message);
   }


   public NtagRawRecord readNDEFCustomAll(byte start) throws IOException, FormatException {
      int ndefsize;
      int tlvsize;
      int tlvPlusNdef;

      // get TLV  // note: [TLV] Tag-Length-Value
      byte[] tlv = readEEPROM(start, start + 3);
      SharedCodes.Companion.log("readCustomAll", "start at " + start);
      NdefMessage message = null;
      boolean isValidNdefMessage = tlv[0] == 0x03;
      ...
      // Read NDEF Message
      byte[] data = readEEPROM(start,start + (tlvPlusNdef / 4));

      SharedCodes.Companion.log("readNDEFCustomAll", "isValidNdefMessage:" + isValidNdefMessage);
      if (isValidNdefMessage){
         byte[] tempdata = Arrays.copyOf(data, data.length);
         // delete TLV
         tempdata = Arrays.copyOfRange(tempdata, tlvsize, tempdata.length);
         // delete end of String which is not part of the NDEF Message
         tempdata = Arrays.copyOf(tempdata, ndefsize);
         SharedCodes.Companion.log("readNDEFCustomAll", "data:" + bytesToHex(tempdata));
         message = new NdefMessage(tempdata);
      }
      // get the String out of the Message
      return new NtagRawRecord(tlvPlusNdef, tlv[0] == 0x03, tlvsize, ndefsize, data, message);
   ...
   public Boolean checkPTwritePossible() throws IOException, FormatException {
		/*
		PT: abbreviate for PASS-TROUGH mode
