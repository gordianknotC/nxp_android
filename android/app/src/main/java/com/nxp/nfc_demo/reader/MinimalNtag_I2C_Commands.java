...
public class MinimalNtag_I2C_Commands extends I2C_Enabled_Commands {
   ...
   public enum Register {
      ...
      Custom_User_memory_Begin((byte) 0x0D), // 2(lang) + 64(text) + 1 = 67 => 9
      UID((byte) 0x00);
   ...
   public void writeEEPROMCustomBytes(byte[] data, byte startAt) throws IOException, FormatException,
      ...
      byte[] temp;
      int blockNr = startAt;
      SharedCodes.Companion.log("MINI2C", "writeEEPROM start at address: " + blockNr);
   ...
   public void writeEEPROM(byte[] data, WriteEEPROMListener listener) throws IOException, FormatException,
      ...
      for (int i = 0; i < data.length; i += 4) {
         temp = Arrays.copyOfRange(data, i, i + 4);
         SharedCodes.Companion.log("writePage", temp + " " + blockNr);
         mfu.writePage(blockNr, temp);
         blockNr++;
   ...
   public void writeEEPROMCustom(byte[] data, WriteEEPROMListener listener) throws IOException, FormatException,
           CommandNotSupportedException {
      if ((tagType == Prod.NTAG_I2C_2k || tagType == Prod.NTAG_I2C_2k_Plus) && data.length > firstSectorMemsize) {
         throw new CommandNotSupportedException(
                 "writeEEPROM is not Supported for this Phone, with Data bigger then First Sector("
                         + firstSectorMemsize + " Bytes)");
      }
      ...
      int blockNr = startAt;
      SharedCodes.Companion.log("MINI2C", "writeEEPROMCustom data:" + new String(data));
      SharedCodes.Companion.log("MINI2C", "writeEEPROMCustom start at address:" + blockNr);
      // write till all Data is written
      for (int i = 0; i < data.length; i += 4) {
         temp = Arrays.copyOfRange(data, i, i + 4);
         mfu.writePage(blockNr, temp);
    ...
    */
   @Override
   public void writeEEPROM(int startAddr, byte[] data) throws IOException, FormatException {
   ...
   public byte[] readEEPROMCustomBytes(int absStart, int absEnd) throws IOException,
      ...
      int i;
      for (i = absStart; i <= (absEnd - 3); i += 4) {
         temp = mfu.readPages(i); // return 4 pages = 16 bytes
         answer = concat(answer, temp);
      }
   ...
   public void writeNDEF(NdefMessage message, WriteEEPROMListener listener) throws IOException,
      ...
      SharedCodes.Companion.log("writeMiniNDEF", "message:" + message);
   ...
   public void writeNDEFCustom(NdefMessage message, WriteEEPROMListener listener) throws IOException,
           FormatException, CommandNotSupportedException {
      byte[] ndefMessageByte = createRawNdefTlv(message);
      SharedCodes.Companion.log("writeMiniNDEFCustom", "message" + message);
      writeEEPROMCustom(ndefMessageByte, listener);
   }
   ...
   public NdefMessage readNDEF() throws IOException, FormatException, CommandNotSupportedException {
      ...
      SharedCodes.Companion.log("readMiniNormal", "start at " + Register.User_memory_Begin.getValue());
    ...
    */
   @Override
   public NtagRawRecord readNDEFCustom() throws IOException, FormatException, CommandNotSupportedException {
      int ndefsize;
      int tlvsize;
      int tlvPlusNdef;
      ...
      SharedCodes.Companion.log("readMiniNDEFCustom", "start at " + startAt);
      ...
      // Read NDEF Message
      byte[] data = readEEPROM(startAt,
              startAt + (tlvPlusNdef / 4));
