...
public class Ntag_Get_Version {
	...
	private void init(Ntag_Get_Version instance){
		vendor_ID = instance.getVendor_ID();
		product_type = instance.getProduct_type();
		product_subtype = instance.getProduct_subtype();
		major_product_version = instance.getMajor_product_version();
		minor_product_version = instance.getMinor_product_version();
		storage_size = instance.getStorage_size();
		protocol_type = instance.getProtocol_type();
	}

	public Ntag_Get_Version (Ntag_Commands reader) throws IOException {
		//untested:
		//gordianknot
		try {
			init(new Ntag_Get_Version(reader.getVersion()));
		} catch (Exception e) {
			e.printStackTrace();
			try {
				reader.close();
				...
				if (temp[0] == (byte) 0x04 && temp[12] == (byte) 0xE1
					...
					init(Ntag_Get_Version.NTAG_I2C_1k);
						...
						&& temp[14] == (byte) 0xEA
						&& temp[15] == (byte) 0x00) {
					init(Ntag_Get_Version.NTAG_I2C_2k);
				}
