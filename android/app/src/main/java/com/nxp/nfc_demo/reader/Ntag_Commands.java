...
public class Ntag_Commands {
	...
	/*
	Ways to get NFC tag
	1) Tag tag = getIntent().getParcelableExtra(NfcAdapter.EXTRA_TAG);
	2) Tag tag = nfcIntent.getParcelableExtra(NfcAdapter.EXTRA_TAG);
	 */
	/**
	...
	public void SectorSelect(byte sector) throws IOException, FormatException {
		...
		/*
		SECTOR_SELECT composed of two pack
		- pack one
			1) sector_select: c2 [1byte]
			2) CRC: ff [1byte]
		- pack two
			1) a related sector(1byte)
			2) RFU (3bytes)
		timeout of SECTOR_SELECT: 5ms (hardware)
		 */
		// pack one
		command = new byte[2];
		command[0] = (byte) 0xc2;
	...
	* @param blockNr  Block Number to write
	* @throws IOException
	* @throws FormatException
	*/
	...
	*/
	...
	public byte[] read(byte blockNr) throws IOException, FormatException {
        ...
        the end of the accessible memory area.
	     */
		command = new byte[2];
